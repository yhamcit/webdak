...
class DatabaseEnvironment(object):
    ...
    __ADDR_URI__ = "add_uri"
    __URI__ = "uri"
    __AUTHID__ = "authid"
    __AUTHPASS__ = "authpass"

    def __init__(self):
        self._db_envs = dict()

    def __iter__(self) -> tuple:
        return ((id, env) for id, env in self._db_envs.items())
    
    @property
    def environment(self) -> dict:
        return self._db_envs
    
    @environment.setter
    def environment(self, profiles):
        # for each database section create environment for int.
        for profile in profiles:
            db_env = DatabaseProfile(**profile)
            self._db_envs[db_env.identifier] = db_env
...
class DatabaseProfile(object):
    ...
    def __init__(self, type, name, addr_uri = "", uri = None, authid = "", authpass = "", **kwargs) -> None:
        ...
        try:
            addr_loc = DatabaseProfile.packnetloc(**uri[DatabaseProfile.__NETLOC__])
