...
class TplusAppTokenHttpCall(HttpCall):
    ...
    __exchange_token_header_filter = HttpHeaders._HTTP_DEFAULT_HEADERS_LIST_ + (
        HttpHeaders._HDR_CONTENT_TYPE_, 
        TplusOpenApiProperties._APP_KEY_, 
        TplusOpenApiProperties._APP_SECRET_
    )
    __exchange_token_cust_headers = HttpHeaders._HTTP_DEFAULT_HEADERS_ | {
    ...
    def refresh_app_token(self, response: httpx.Response):
